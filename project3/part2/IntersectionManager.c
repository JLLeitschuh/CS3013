 ...
 */
 void allowCarEntry(IntersectionQuadrant_t quadrant){
  ...
  //convert it to a cardinal entry pt.
  //check on the queue
  //remove1stvehicle and ul=nblock its mutex, and wait for 
  //the car to pass (wait while 1 for the car to the ) 


  //Move the car into the intersection
  //Unlock the cars queue mutex to allow it into the intersection
  sem_post(this_vehicle->queueLock);
}//lhnguyen
